...
try:
    ...
    sys.stderr.write('usage: script.py filename colnum0 colnum1 out.png [pval]\n')
...
for cat in cats:
    ...
    bp = panel1.violinplot(normalized_exp, \
        positions=[xaxis_pos], \
        #patch_artist=True, \
        widths=0.5)
    for box in bp['bodies']:
        box.set(edgecolor='black', facecolor='#8c510a', linewidth=0.75)
    bp['cbars'].set(color='gray', linestyle='-', linewidth=0.75)
    bp['cmaxes'].set(color='none', linestyle='-', linewidth=0.75)
    bp['cmins'].set(color='none', linestyle='-', linewidth=0.75)
    panel1.scatter(xaxis_pos, np.median(normalized_exp), color='gray', s=2)
...
if not pval:
    wilcoxon = stats.mannwhitneyu(values[cats[0]], values[cats[1]])
    print(wilcoxon)
    pval = str(wilcoxon[1])
