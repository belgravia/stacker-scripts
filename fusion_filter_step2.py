...
def validFusion(s1, e1, s2, e2):
	...
	if s1 - 10 < s2 and s1 + 10 > s2:  # if s2 is +/- 10 of s1
		return False
	if (s2 < e1 - 200 and s2 > s1) or (s1 < e2 - 200 and s1 > s2):  #  any more overlap than 10 is bad
		return False
...
def process_entries(entries, line, name):
		...
		entries = [e for e in entries if int(e[0]) > 200]
...
with open(sys.argv[1][(sys.argv[1].rfind('/')+1):-4] + '_fusions_step2.psl', 'wt') as csvfile:
